...
int main (int argc, char *argv[])
{
    ...
    if ( argc < 5 ) {
        ...
        puts ("   Computes the sum of Frobenius traces a_p of y^2 = x^3 + ax + b for p in [17,N] not dividing b*(4a^3+27b^2).");
        puts ("   The purpose of this code is to demonstrate/test the rforest library.  If you actually want to compute traces of an elliptic curve over Q, use smalljac, it is at least 10x faster.");   
        return 0;
    }
    ...
    mpz_mul_si (D, D, b); // multiply D by b so that we will skip p | b below
    ...
    // compute the sum of a_p for p in [17,N] not dividing D = b(4a^3+27b^2) by lifting last entry in each A_i to Z
    long sum = 0;
    mpz_set_si(w,b);
    for ( i = 0 ; i < n ; i++ ) {
    ...
    printf ("Trace sum over p in [17,%ld] not dividing b*(4a^3+27b^2) is %ld\n", N, sum);
